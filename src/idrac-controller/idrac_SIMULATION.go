...
import (
	...
	idrac_attribute "github.com/superchalupa/go-redfish/src/idrac-mv/attribute"
)
...
func New(ctx context.Context, logger log.Logger, cfgMgr *viper.Viper, viperMu *sync.Mutex, ch eh.CommandHandler, eb eh.EventBus, ew *utils.EventWaiter) *ocp {
	...
	attrSvc, _ := idrac_attribute.New(
		idrac_attribute.InResource(bmcSvc, "system.embedded.1"),
		idrac_attribute.WithAttribute("testgroup", "1", "A", "happy"),
		idrac_attribute.WithAttribute("testgroup", "1", "B", "joy"),
		idrac_attribute.WithAttribute("testgroup", "2", "A", "happy 2"),
		idrac_attribute.WithAttribute("testgroup", "2", "B", "joy 2"),
		idrac_attribute.WithAttribute("test", "1", "C", "foo"),
	)
	...
	domain.RegisterPlugin(func() domain.Plugin { return attrSvc })
	...
	attrSvc.AddView(ctx, ch, eb, ew)
	attrSvc.AddController(ctx, ch, eb, ew)
