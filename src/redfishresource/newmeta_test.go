...
func TestParsing(t *testing.T) {
	var tests = []struct {
		...
		{"test slice 1", &RedfishResourceProperty{Value: []string{"happy joy"}}, []interface{}{"happy joy"}},
		{"test map 1", &RedfishResourceProperty{Value: map[string]string{"happy": "joy"}}, map[string]interface{}{"happy": "joy"}},
		{"test recursion 1",
	...
	for _, subtest := range tests {
		t.Run(subtest.testname, func(t *testing.T) {
			...
			output := Flatten(subtest.input.Value, false)
